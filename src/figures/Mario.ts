...
export class Mario extends Figure implements DeathAnimation {
  ...
  standSprites: Array<Array<Array<Point>>>;
  ...
  constructor(level: Level) {
    ...
    this.crouchSprites = [[{ x: 241, y: 0 }, { x: 161, y: 0 }], [{ x: 241, y: 162 }, { x: 241, y: 243 }]];
    this.deadly = 0;
    this.invulnerable = 0;
    this.width = 80;
    this.blinking = 0;
  ...
  restore(settings: Settings) {
    ...
    this.setMarioState(settings.marioState);
  }
  ...
  stand() {
    const coords = this.standSprites[this.state - 1][this.direction === Direction.left ? 0 : 1][this.onground ? 0 : 1];
    this.setImage(images.sprites, coords.x, coords.y);
    this.clearFrames();
  }

  crouch() {
    const coords = this.crouchSprites[this.state - 1][this.direction === Direction.left ? 0 : 1];
    this.setImage(images.sprites, coords.x, coords.y);
    this.clearFrames();
  }
